...
class TestValidator(unittest.TestCase):
    def setUp(self):
        # Base valid manifest structure (schemaVersion 2)
        self.valid_manifest = {
            "schemaVersion": 2,
            "reference": {
            ...
            "corpusDigest": {
                "M1a": "sha256:1337a96b4996adb6a4ec87f8819ee2421385299fbc568a6e86af224cbf71d8f0"
            },
            "requiredCaseIds": {
                "M0": [],
                "M1a": ["case_1"],
    ...
    def test_valid_manifest_passes(self):
        errors = validate_manifest(self.valid_manifest, canonical_m1a_ids=["case_1"])
        self.assertEqual(errors, [])

    def test_missing_schema_version(self):
        manifest = dict(self.valid_manifest)
        del manifest["schemaVersion"]
        errors = validate_manifest(manifest, canonical_m1a_ids=["case_1"])
    ...
    def test_invalid_schema_version(self):
        ...
        errors = validate_manifest(manifest, canonical_m1a_ids=["case_1"])
        self.assertIn("Invalid schemaVersion: 1", errors)
    ...
    def test_missing_corpus_digest(self):
        manifest = dict(self.valid_manifest)
        del manifest["corpusDigest"]
        errors = validate_manifest(manifest, canonical_m1a_ids=["case_1"])
        self.assertTrue(any("Missing or invalid corpusDigest" in e for e in errors))

    def test_malformed_corpus_digest(self):
        manifest = dict(self.valid_manifest)
        manifest["corpusDigest"] = {"M1a": "invalid-digest"}
        errors = validate_manifest(manifest, canonical_m1a_ids=["case_1"])
        self.assertTrue(any("corpusDigest.M1a must be in format" in e for e in errors))
    ...
    def test_milestone_uncaptured_checks(self):
        ...
        errors_gen = validate_manifest(self.valid_manifest, canonical_m1a_ids=["case_1"])
        self.assertEqual(errors_gen, [])

        # M0 validation allows pending capture (since M0 has no cases in self.valid_manifest)
        errors_m0 = validate_manifest(self.valid_manifest, milestone="M0", canonical_m1a_ids=["case_1"])
        self.assertEqual(errors_m0, [])

        # M1a validation fails since case_1 is not captured
        errors_m1a = validate_manifest(self.valid_manifest, milestone="M1a", canonical_m1a_ids=["case_1"])
        ...
        errors_captured = validate_manifest(manifest_captured, milestone="M1a", canonical_m1a_ids=["case_1"])
